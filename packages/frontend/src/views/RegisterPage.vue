...
<template>
  <div class="auth-page">
    <div class="auth-card">
      <h1>{{ AUTH_LABELS.REGISTER_TITLE }}</h1>
      ...
      <form @submit.prevent="handleSubmit">
        ...
        <div class="form-group">
          <label for="name">{{ AUTH_LABELS.NAME }}</label>
          <input
            id="name"
            v-model="name"
            type="text"
            :placeholder="AUTH_LABELS.PLACEHOLDER_NAME"
          />
        </div>
        ...
        <div class="form-group">
          <label for="passwordConfirm">{{ AUTH_LABELS.PASSWORD_CONFIRM }} *</label>
          <input
            id="passwordConfirm"
        ...
        <button type="submit" class="submit-btn" :disabled="loading">
          {{ loading ? '...' : AUTH_LABELS.REGISTER }}
        </button>
      </form>

      <div class="auth-footer">
        <span>{{ AUTH_LABELS.HAS_ACCOUNT }}</span>
